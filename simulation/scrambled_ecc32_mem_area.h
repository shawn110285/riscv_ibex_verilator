...
class ScrambledEcc32MemArea : public Ecc32MemArea 
{
 public:
   ...
   *
   */
  ScrambledEcc32MemArea(const std::string &scope, uint32_t size, uint32_t width_32, bool repeat_keystream = true);

 private:
  void WriteBuffer(uint8_t buf[SV_MEM_WIDTH_BYTES], const std::vector<uint8_t> &data, size_t start_idx, uint32_t dst_word) const override;

  void ReadBuffer(std::vector<uint8_t> &data, const uint8_t buf[SV_MEM_WIDTH_BYTES], uint32_t src_word) const override;
