...
bool VerilatorMemUtil::ParseCLIArguments(int argc, char **argv, bool &exit_app) 
{
  ...
  while (1) 
  {
    ...
    switch (c) 
    {
      ...
      case 'r':
        load_args.push_back({.name = "rom", .filepath = optarg, .type = kMemImageUnknown});
        break;

      case 'm':
        load_args.push_back({.name = "ram", .filepath = optarg, .type = kMemImageUnknown});
        break;

      case 'f':
        load_args.push_back({.name = "flash", .filepath = optarg, .type = kMemImageUnknown});
        break;

      case 'o':
        load_args.push_back({.name = "otp", .filepath = optarg, .type = kMemImageUnknown});
        break;
      ...
      case 'E':
        load_args.push_back({.name = "", .filepath = optarg, .type = kMemImageElf});
        break;
  ...
  for (const LoadArg &arg : load_args) 
  {
    try 
    {
      if (!arg.name.empty()) 
      {
        mem_util_->LoadFileToNamedMem(verbose, arg.name, arg.filepath, arg.type);
      } 
      else 
      {
