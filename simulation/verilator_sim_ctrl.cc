...
void VerilatorSimCtrl::SetTop(VerilatedToplevel *top, CData *sig_clk, CData *sig_rst, VerilatorSimCtrlFlags flags) 
{
  top_ = top;
  sig_clk_ = sig_clk;
  sig_rst_ = sig_rst;
...
static bool read_ul_arg(unsigned long *arg_val, const char *arg_name, const char *arg_text) 
{
...
void VerilatorSimCtrl::RunSimulation() 
{
  ...
  if (TracingPossible()) 
  {
    std::cout << "Tracing can be toggled by sending SIGUSR1 to this process:" << std::endl
...
void VerilatorSimCtrl::Run() 
{
  ...
  std::cout << std::endl << "Simulation running, end by pressing CTRL-c." << std::endl;
  ...
  while (1) 
  {
    ...
    *sig_clk_ = !*sig_clk_;
    ...
    if (request_stop_) 
    {
      std::cout << "Received stop request, shutting down simulation." << std::endl;
      break;
    }

    if (Verilated::gotFinish()) 
    {
      std::cout << "Received $finish() from Verilog, shutting down simulation." << std::endl;
      break;
    }

    if (term_after_cycles_ && (time_ / 2 >= term_after_cycles_)) 
    {
      std::cout << "Simulation timeout of " << term_after_cycles_ << " cycles reached, shutting down simulation." << std::endl;
      break;
    }
  }
...
{
  return std::chrono::duration_cast<std::chrono::milliseconds>(time_end_ - time_begin_)
      .count();
}
...
void VerilatorSimCtrl::Trace() 
{
  ...
  if (!tracer_.isOpen()) 
  {
    ...
    std::cout << "Writing simulation traces to " << GetTraceFileName() << std::endl;
  }
