...
static std::vector<uint8_t> byte_reverse_vector(const std::vector<uint8_t> &vec_in) 
{
...
static uint8_t read_vector_bit(const std::vector<uint8_t> &vec, uint32_t bit_pos) 
{
...
static void or_vector_bit(std::vector<uint8_t> &vec, uint32_t bit_pos, uint8_t bit) 
{
  assert(bit_pos / 8 < vec.size());
...
static std::vector<uint8_t> xor_vectors(const std::vector<uint8_t> &vec_a, const std::vector<uint8_t> &vec_b) 
{
  assert(vec_a.size() == vec_b.size());
  std::vector<uint8_t> vec_out(vec_a.size());
  std::transform(vec_a.begin(), vec_a.end(), vec_b.begin(), vec_out.begin(), std::bit_xor<uint8_t>{});
  return vec_out;
}
...
static std::vector<uint8_t> scramble_sbox_layer(const std::vector<uint8_t> &in, uint32_t bit_width, uint8_t sbox[16]) 
{
...
static std::vector<uint8_t> scramble_flip_layer(const std::vector<uint8_t> &in, uint32_t bit_width) 
{
  assert(in.size() == ((bit_width + 7) / 8));
  std::vector<uint8_t> out(in.size(), 0);
