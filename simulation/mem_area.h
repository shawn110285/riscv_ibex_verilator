...
class MemArea 
{
 public:
   ...
   *                    zero-extended.
   */
  virtual void Write(uint32_t word_offset, const std::vector<uint8_t> &data) const;
   ...
   */
  virtual std::vector<uint8_t> Read(uint32_t word_offset, uint32_t num_words) const;
 ...
 protected:
   ...
   */
  virtual void WriteBuffer(uint8_t buf[SV_MEM_WIDTH_BYTES], const std::vector<uint8_t> &data, size_t start_idx, uint32_t dst_word) const;
   ...
   */
  virtual void ReadBuffer(std::vector<uint8_t> &data, const uint8_t buf[SV_MEM_WIDTH_BYTES], uint32_t src_word) const;
