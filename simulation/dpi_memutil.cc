...
static std::vector<uint8_t> FlattenElfFile(const std::string &filepath) 
{
  ...
  for (size_t i = 0; i < phnum; i++) 
  {
    ...
    {
      std::cout << "Program header number " << i << " in `" << filepath << "' is not of type PT_LOAD; ignoring." << std::endl;
      continue;
    }
...
void DpiMemUtil::RegisterMemoryArea(const std::string &name, uint32_t base, const MemArea *mem_area) 
{
  assert(mem_area);
...
MemImageType DpiMemUtil::GetMemImageType(const std::string &path, const char *type) 
{
...
void DpiMemUtil::LoadFileToNamedMem(bool verbose, const std::string &name, const std::string &filepath, MemImageType type) 
{
  ...
  {
    std::ostringstream oss;
    oss << "`" << name << ("' is not the name of a known memory region. Run with --meminit=list to get a list.");
    throw std::runtime_error(oss.str());
  }

  if (verbose) 
  {
    std::cout << "Loading data from file `" << filepath << "' into memory `" << name << "'." << std::endl;
  }
  ...
  {
    std::ostringstream oss;
    oss << "No memory found at `" << err.scope_name_ << "' (the scope associated with region `" << name << "').";
    throw std::runtime_error(oss.str());
  }
}
...
void DpiMemUtil::LoadElfToMemories(bool verbose, const std::string &filepath) 
{
  ...
  for (const auto &pr : staging_area_) 
  {
    ...
    for (const auto &seg_pr : staged_mem.GetSegs()) 
    {
      ...
      {
        std::ostringstream oss;
        oss << "No memory found at `" << err.scope_name_ << "' (the scope associated with region `" << mem_name
            << "', used by a segment that starts at LMA 0x" << std::hex << base_addrs_[mem_area_it->second] + seg_rng.lo << ").";
        throw std::runtime_error(oss.str());
      }
    }
  }
...
void DpiMemUtil::StageElf(bool verbose, const std::string &path) 
{
  ...
  for (size_t i = 0; i < phnum; ++i) 
  {
    ...
    size_t mem_area_idx = GetRegionForSegment(path, i, phdr.p_paddr, phdr.p_filesz);
    ...
    if (verbose) 
    {
      std::cout << "Loading segment " << i << " from ELF file `" << path << "' into memory `" << name << "'." << std::endl;
    }
...
size_t DpiMemUtil::GetRegionForSegment(const std::string &path, int seg_idx, uint32_t lma, uint32_t mem_sz) const 
{
  assert(mem_sz > 0);
