...
static std::vector<uint8_t> ByteVecFromSV(svBitVecVal sv_val[], uint32_t bytes) 
{
  int shift = 0;
...
ScrambledEcc32MemArea::ScrambledEcc32MemArea(const std::string &scope, uint32_t size, uint32_t width_32, bool repeat_keystream)
    : Ecc32MemArea(SVScoped::join_sv_scopes(scope, "u_prim_ram_1p_adv.u_mem.gen_generic.u_impl_generic"), size, width_32), 
      scr_scope_(scope) 
{
...
void ScrambledEcc32MemArea::WriteBuffer(uint8_t buf[SV_MEM_WIDTH_BYTES], const std::vector<uint8_t> &data, size_t start_idx, uint32_t dst_word) const 
{
  // Compute integrity
  Ecc32MemArea::WriteBuffer(buf, data, start_idx, dst_word);
  std::vector<uint8_t> scramble_buf = std::vector<uint8_t>(buf, buf + GetPhysWidthByte());

  // Scramble data with integrity
  scramble_buf = scramble_encrypt_data(scramble_buf, GetPhysWidth(), 39, AddrIntToBytes(dst_word, addr_width_), 
...
void ScrambledEcc32MemArea::ReadBuffer(std::vector<uint8_t> &data, const uint8_t buf[SV_MEM_WIDTH_BYTES], uint32_t src_word) const 
{
  // Unscramble data from read buffer
  std::vector<uint8_t> scrambled_data = std::vector<uint8_t>(buf, buf + GetPhysWidthByte());
  std::vector<uint8_t> unscrambled_data = scramble_decrypt_data(scrambled_data, GetPhysWidth(), 39, AddrIntToBytes(src_word, addr_width_),
                                                                addr_width_, GetScrambleNonce(), GetScrambleKey(), repeat_keystream_);
...
uint32_t ScrambledEcc32MemArea::ToPhysAddr(uint32_t logical_addr) const 
{
  ...
  return AddrBytesToInt(scramble_addr(AddrIntToBytes(logical_addr, addr_width_), addr_width_, GetScrambleNonce(), GetNonceWidth()));
}
