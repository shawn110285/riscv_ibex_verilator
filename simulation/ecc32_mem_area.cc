...
Ecc32MemArea::Ecc32MemArea(const std::string &scope, uint32_t size, uint32_t width_32) : MemArea(scope, size, 4 * width_32) 
{
...
{
  throw std::runtime_error("vmem files are not supported for memories with ECC bits");
}



void Ecc32MemArea::WriteBuffer(uint8_t buf[SV_MEM_WIDTH_BYTES], const std::vector<uint8_t> &data, size_t start_idx, uint32_t dst_word) const 
{
...
void Ecc32MemArea::ReadBuffer(std::vector<uint8_t> &data, const uint8_t buf[SV_MEM_WIDTH_BYTES], uint32_t src_word) const 
{
