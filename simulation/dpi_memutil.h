...
class DpiMemUtil 
{
 public:
   ...
   */
  void RegisterMemoryArea(const std::string &name, uint32_t base, const MemArea *mem_area);
   ...
   */
  static MemImageType GetMemImageType(const std::string &path, const char *type);
   ...
   */
  void LoadFileToNamedMem(bool verbose, const std::string &name, const std::string &filepath, MemImageType type);
 ...
 private:
   ...
   */
  size_t GetRegionForSegment(const std::string &path, int seg_idx, uint32_t lma, uint32_t mem_sz) const;
};
