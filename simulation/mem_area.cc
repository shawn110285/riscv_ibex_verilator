...
MemArea::MemArea(const std::string &scope, uint32_t num_words, uint32_t width_byte) : scope_(scope), num_words_(num_words), width_byte_(width_byte) 
{
  assert(0 < num_words);
...
void MemArea::Write(uint32_t word_offset, const std::vector<uint8_t> &data) const 
{
  ...
  for (uint32_t i = 0; i < data_words; ++i) 
  {
    ...
    if (!simutil_set_mem(phys_addr, (svBitVecVal *)minibuf)) 
    {
      ...
      oss << "Could not set memory at byte offset 0x" << std::hex << dst_word * width_byte_ << ".";
...
std::vector<uint8_t> MemArea::Read(uint32_t word_offset, uint32_t num_words) const 
{
  ...
  for (uint32_t i = 0; i < num_words; ++i) 
  {
    ...
    uint32_t phys_addr = ToPhysAddr(src_word);
    {
      ...
      {
        std::ostringstream oss;
        oss << "Could not read memory at byte offset 0x" << std::hex  << src_word * width_byte_ << ".";
        throw std::runtime_error(oss.str());
      }
    }
...
void MemArea::WriteBuffer(uint8_t buf[SV_MEM_WIDTH_BYTES], const std::vector<uint8_t> &data, size_t start_idx, uint32_t dst_word) const 
{
...
void MemArea::ReadBuffer(std::vector<uint8_t> &data,  const uint8_t buf[SV_MEM_WIDTH_BYTES], uint32_t src_word) const 
{
  // Append the first width_byte_ bytes of buf to data.
  std::copy_n(reinterpret_cast<const char *>(buf), width_byte_, std::back_inserter(data));
}
